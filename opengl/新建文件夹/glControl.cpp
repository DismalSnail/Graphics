...
bool CglControl::MouseCtrlView(int message, CPoint pt)
{
	...
	if(message==WM_MOUSEMOVE)
	{
		if(m_bMouseLDown)//移动
		{
			CPoint dis;
			dis = pt - m_MousePos;
			m_pCamera->Move(0,dis.x*mspeed);
			m_pCamera->Move(1,-dis.y*mspeed);
			m_MousePos = pt;
			ret = true;
		}
		else if(m_bMouseRDown)//旋转
		{
			CPoint dis;
			dis = pt - m_MousePos;
			m_pCamera->Rotate(0,-dis.x*rspeed);
			m_pCamera->Rotate(1,-dis.y*rspeed);
			m_MousePos = pt;
			ret = true;
		}
	}
...
bool CglControl::KeyboardCtrlView()
{
	...
	{
		m_pCamera->Move(2,m_fKeyStep*mspeed);
		bHaveKeyDown = true;
	}
	...
	{
		m_pCamera->Move(0,-m_fKeyStep*mspeed);
		bHaveKeyDown = true;
	}
	...
	{
		m_pCamera->Move(1,m_fKeyStep*mspeed);
		bHaveKeyDown = true;
	}
	...
	{
		m_pCamera->Rotate(2,m_fKeyStep*rspeed);
		bHaveKeyDown = true;
	}
	...
	{
		m_pCamera->Rotate(0,m_fKeyStep*rspeed);
		bHaveKeyDown = true;
	}
	...
	{
		m_pCamera->Rotate(1,m_fKeyStep*rspeed);
		bHaveKeyDown = true;
	}
